...
# from mcts import MCTS as MCTSRs
# from mcts import Node as NodeRs
from mcts_rs import MCTSRs
...
class DiagnoseModel:
    ...
    def get_virtual_trajectory_from_obs(
        ...
        mcts_rs = MCTSRs(self.config.__dict__)
        root, mcts_info = mcts_rs.run(
            self.model, observation, self.config.action_space, to_play, True
        )
        trajectory_info.store_info(mcts_rs, root, mcts_info, None, numpy.NaN)

        virtual_to_play = to_play
        for i in range(horizon):
            action = SelfPlay.select_action(mcts_rs, root, 0)
            ...
            trajectory_info.store_info(
                mcts_rs, root, mcts_info, action, reward, new_prior_root_value=value
            )
    ...
    def compare_virtual_with_real_trajectories(
        ...
        mcts_rs = MCTSRs(self.config.__dict__)
        root, mcts_info = mcts_rs.run(
            self.model, first_obs, game.legal_actions(), game.to_play(), True
        )
        self.plot_mcts(mcts_rs, root.idx, plot)
        real_trajectory_info.store_info(mcts_rs, root, mcts_info, None, numpy.NaN)
        for i, action in enumerate(virtual_trajectory_info.action_history):
            ...
            real_trajectory_info.store_info(mcts_rs, root, mcts_info, action, reward)
            if done:
    ...
    def plot_mcts(self, mcts_rs, root, plot=True):
        """
        ...
        def traverse(node, action, parent_id, best):
            ...
            graph.node(
                str(node_id),
                label=f"Action: {action}\nValue: {mcts_rs.get_node(node).value():.2f}\nVisit count: {mcts_rs.get_node(node).visit_count}\nPrior: {mcts_rs.get_node(node).prior:.2f}\nReward: {mcts_rs.get_node(node).reward:.2f}",
            ...
            if len(mcts_rs.get_children(node)) != 0:
                best_visit_count = max(
                    [child.visit_count for child in mcts_rs.get_children(node).values()]
                )
            else:
                best_visit_count = False
            for action, child in mcts_rs.get_children(node).items():
...
class Trajectoryinfo:
    ...
    def store_info(
        self, mcts_rs, root, mcts_info, action, reward, new_prior_root_value=None
    ):
        ...
        self.prior_policies.append(
            [
                mcts_rs.get_children(root.idx)[action].prior
                if action in mcts_rs.get_children(root.idx).keys()
                else numpy.NaN
        ...
        self.policies_after_planning.append(
            [
                mcts_rs.get_children(root.idx)[action].visit_count
        ...
        self.values_after_planning.append(
            [
                mcts_rs.get_children(root.idx)[action].value()
                if action in mcts_rs.get_children(root.idx).keys()
                else numpy.NaN
                for action in self.config.action_space
            ]
        ...
        self.prior_rewards.append(
            [
                mcts_rs.get_children(root.idx)[action].reward
                if action in mcts_rs.get_children(root.idx).keys()
                else numpy.NaN
                for action in self.config.action_space
            ]
